...
adjustSampleInterval2 :: Monad m => NanoSeconds -> ResampleStrategy -> -- Conduit (Time, Value) m (Value, AdjustSampleState)
                                                                       ConduitM (Time, Value) Value m ((), AdjustSampleState)
adjustSampleInterval2 sampleTime resampleStrategy = runStateC startAdjustSampleState $ awaitTwo go
  where go (t1, v1) (t2, v2) = do
